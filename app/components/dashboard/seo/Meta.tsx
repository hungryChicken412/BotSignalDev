			...
			<div className="p-6 space-y-6">
					...
					<div className="flex justify-between items-end mb-2">
						...
						<span
							className={`font-mono text-xs font-medium ${
								metadata.title.isOptimal
									? "text-emerald-600"
									: "text-amber-600"
							}`}
						>
							{metadata.title.length} / {metadata.title.maxLength}{" "}
							chars
						</span>
					</div>
				...
				<div className="group">
					<div className="flex justify-between items-end mb-2">
						...
						<span
							className={`font-mono text-xs font-medium ${
								metadata.description.isOptimal
									? "text-emerald-600"
									: "text-amber-600"
							}`}
						>
							{metadata.description.length} /{" "}
							{metadata.description.maxLength} chars
						</span>
					</div>
				...
				<div className="flex items-center justify-between rounded-2xl border border-neutral-200 bg-white p-4">
					<div className="truncate pr-4">
						...
						<code className="font-mono text-xs text-neutral-900 truncate block">
							{metadata.canonical.url}
						</code>
					</div>

					<span
						className={`inline-flex items-center gap-1 rounded-2xl border px-3 py-1.5 text-xs font-medium shrink-0 ${
							metadata.canonical.isOptimal
